...
get '/charts' do
  ...
  get_keywords().each do | keyword |
    ...
    ret_string = ret_string + '<p><b>' + keyword + '</br><img src="' + Gchart.line(:data => bar_data, :axis_with_labels => ['x', 'y'], :axis_labels => [labels,'0|200'], :bar_colors => 'FF1111,1111FF', :stacked=>false, :legend => ['Google', 'Bing'] ) + '" title="bar" alt="bar"></p>'
  end
  ret_string
end
