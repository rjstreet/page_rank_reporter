...
get '/charts' do
  ...
  get_keywords().each do | keyword |
    ...
    ret_string = ret_string + '<p><b>' + keyword + '</br><img src="' + Gchart.line(:data => bar_data[0], :axis_with_labels => ['x', 'y'], :axis_labels => labels, :bar_colors => 'FF1111,1111FF', :legend => ['Google', 'Bing'] ) + '" title="bar" alt="bar"></p>'
  end
  ret_string
end
