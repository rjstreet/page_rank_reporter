...
get '/charts' do
  ...
  get_keywords().each do | keyword |
    ...
    ret_string = ret_string + '<p><b>' + keyword + '</br><img src="' + Gchart.line(:data => bar_data, :axis_with_labels => 'y', :bar_colors => 'FF1111,1111FF', :legend => ['Google', 'Bing'], :custom => 'chg=10,15,1,0' ) + '" title="bar" alt="bar"></p>'
  end
  ret_string
end
